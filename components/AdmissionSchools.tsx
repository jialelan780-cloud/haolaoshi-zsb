...
/**
 * 代表录取院校：从 2022-2026 真实录取数据动态统计（仅已上岸学生）。
 * 点击院校可展开被该校录取的学生名单。
 */
export default function AdmissionSchools({ teacherName }: { teacherName: string }) {
  const [active, setActive] = useState<string | null>(null);
  const [showAll, setShowAll] = useState(false);

  const groups = useMemo(
    () => getSchoolGroups(filterRecords({ teacher: teacherName })),
    [teacherName],
  );
              ...
              <table className="w-full min-w-[620px] border-collapse text-sm">
                <thead>
                  <tr className="bg-brand-50 text-left text-brand-900">
                    {["年份", "学生", "数学", "英语", "总分", "班级", "专科院校", "录取类型"].map((h) => (
